...
| --- | --- | --- | --- |
| Android app build | `VERIFIED_BUILD` | A1 `assembleDebug` passed on the accepted public source baseline. | A desktop build is not Temi device acceptance. |
| JVM tests | `VERIFIED_UNIT` | A1 fresh-clone `:app:testDebugUnitTest`: `288/288` passed. | JVM contracts do not prove hardware, broker, or AI6 behavior. |
| Demo signing | `VERIFIED_BUILD` | Signed `assembleDemo`, `verifyDemoSigningConfig`, and Demo artifact preflight passed; expected signer digest is recorded in [CURRENT_STATUS.md](./CURRENT_STATUS.md). | Private signing assets are out of band; no bit-for-bit APK reproducibility claim. |
| Demo non-debuggable boundary | `VERIFIED_BUILD` | Packaged Demo preflight reported `debuggable=false`. | Debug builds remain development builds and are not the accepted Demo artifact. |
...
| Canonical `play_media` | `VERIFIED_SOURCE` | Validator allowlist, canonical media tracking, and `MainActivity` playback path are present; Media v1.1 parser/runtime tests cover the separate media contract. | Playback depends on private resources and a device binding; no current physical E2E is verified. |
| Optional exercise media | `EXPERIMENTAL` | `ExerciseMediaResourceResolverTest` and Media v1.1 tests cover allowlisting and explicit unavailable-resource handling. | Deployment video files are private and absent from the public repository; actual playback is not verified. |
