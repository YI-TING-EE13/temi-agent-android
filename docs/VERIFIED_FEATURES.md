...
| Project-owned exercise media | `VERIFIED_BUILD` | Both tracked MP4s are present in the debug and Demo APKs; the Demo resource table maps both fixed raw IDs, packaged bytes match the source hashes, and a clean clone contains and rebuilds both resources with `288/288` JVM tests passing. | Physical playback is not `VERIFIED_DEVICE`; V4B is required for device acceptance. |
